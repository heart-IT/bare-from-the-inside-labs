...
things; every probe runs under Bare, and probe 2 also runs
under Node so you can see the two disagree.
...
same throw, one listener
  handled: boom
  -> exit code 0
     the first statement of the policy returns early when a listener handles it
...
position relative to `timeout` changes between runs in Bare, while Node put it after `timeout` in all fifty
of its runs here. A probe
...
Thirteen handles exist before the probe sets its timer — eleven from startup and
two idle handles `bare-dns` creates when `bare-tcp` loads it — and one is both
active and ref'd. Six are unref'd — libjs's task-runner and environment handles, Bare's
