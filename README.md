...
|---|---|---|
| [`lab-01-what-is-bare`](lab-01-what-is-bare) | 1 — Why P2P Needed Its Own Runtime | Five probes: version identity, the `Bare` namespace, what `require()` finds before and after `npm i bare-crypto` and from an empty folder, what `npm i bare` actually installed, and `fetch` arriving as a package (bare-fetch + bare-http1) |
